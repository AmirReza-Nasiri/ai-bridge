...
### Changed
- **CI and release no longer build or ship the `x86_64-unknown-linux-gnu` target** — Windows + macOS only (saves CI quota; rtk was already `Unsupported` on Linux). Linux still builds from source but has no CI coverage and no prebuilt release binary. **Release assets: 6** (was 8) — the two `aibridge-x86_64-unknown-linux-gnu` files are no longer published.
